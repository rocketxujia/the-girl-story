...
$(function () {
    ...
    $('#articles').infinitescroll({
        ...
        $(newElements).find('.gallery').imagesLoaded(function () {
            var $this = $(this);
            $this.filter('.gallery-size-masonry').masonry({
                itemSelector:'.gallery-item',
                columnWidth:182
    ...
    var getCount =function (){
        var $page_a_links = jQuery("a[uyan_identify='true']");

        $page_a_links.each(function(index){
            var $link = $(this), pageUrl =$page_a_links.eq(index).attr('href');
            $.getScript('http://api.uyan.cc/?mode=count&'+ jQuery.param( {
                txt:'0条评论',
                url:pageUrl
            }) , function(){
                $link.attr('uyan_identify','false');
                console.log(jQuery("a[uyan_identify='true']").length);
            });
        });
    };
    getCount();
